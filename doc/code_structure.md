...
- `server`\
 A folder you're [supposed to create](build.md#running-a-node), before starting your server: cd to project root; mkdir server; cd server; epic server start (or run) and it will create a subfolder .epic
  - `.epic`
...
- `wallet`\
  ...
  - `wallet_data` - a database storing your "outputs", that once confirmed and matured, can be spent with the [`epic wallet send`](wallet/usage.md) command. (locally created, *not* in git)
